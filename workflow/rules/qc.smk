...
# Helper functions to resolve dynamic input files
def get_fastq_screen_config(wildcards):
    """
    Returns the FastQ Screen configuration file. This 
    will change depending on whether the --resource-bundle
    option was provided. If it was provided, we will use 
    the config file generated by "rule fastq_screen_config".
    Else, we will use the default config file.
    """
    if create_fastq_screen_config:
        # Resource bundle provided, use the generated config file
        return [join(workpath,"FQscreen","fastq_screen.conf")]
    else:
        # Use the default config file, ony works on Biowulf
        return [join(workpath, config['references']['FASTQ_SCREEN_CONFIG'])]
...
rule fastq_screen_config:
    """
    Quality-control related step to create a FastQ Screen configuration file.
    FastQ Screen requires a configuration file to specify the location of the
    of the bowtie2 indices. This rule creates a configuration file with the
    --resource-bundle PATH to the indices. The 
    @Input:
    ...
    """
    input:
        conf = join(workpath, config['references']['FASTQ_SCREEN_CONFIG']),
    output:
        conf = join(workpath,"FQscreen","fastq_screen.conf"),
    params:
        rname  = "fqscreen_config",
        bundle = config['options']['resource_bundle'],
    container: config['images']['genome-seek_qc'],
    envmodules: 
    ...
    shell: """
    # Run an ls command to check if the path
    # exists, if not, it will error out here 
    # before the new config file is created.
    ls -larth {params.bundle}
    # Create a new FastQ Screen config file 
    # using the user provided path to the 
    # resource bundle. The default location 
    # points to our Biowulf reference files.
    sed 's@/data/OpenOmics/references/genome-seek@{params.bundle}@g' \\
        {input.conf} \\
    > {output.conf}
    """
